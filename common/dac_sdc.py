...
class Team:
    ...
    def run(self, callback, debug=True):
        ...
        while(True):
            ...
            if debug:
                print("Starting batch of", len(rgb_imgs), "images")
            ...
            if len(object_locations) != len(rgb_imgs):
                raise ValueError(str(len(rgb_imgs)) + " images provided, but only " + str(len(object_locations)) + " object locations returned.")
            self.__result_rectangles.extend(object_locations)

            runtime = end - start
            energy = sum([recorder.frame[r + "_power"].mean() for r in rails_to_monitor]) * runtime   

            if (debug):
                print("Done batch.  Runtime =", runtime, "seconds. Energy =", energy, "J.")
